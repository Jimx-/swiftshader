...
ScanlineRasterizer::ScanlineRasterizer(const PixelProcessor::State &state, const SpirvShader *spirvShader)
    : state(state)
...
void ScanlineRasterizer::generate()
{
...
void ScanlineRasterizer::rasterize(Int &yMin, Int &yMax)
{
...
SIMD::Float ScanlineRasterizer::interpolate(SIMD::Float &x, SIMD::Float &D, SIMD::Float &rhw, Pointer<Byte> planeEquation, bool flat, bool perspective)
{
	if(flat)
	{
		return D;
